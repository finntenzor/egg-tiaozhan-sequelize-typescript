...
module.exports = app => {
  ...
  /**
   * determine if the model a sequelize model
   * @param {Object} model model class
   */
  function isSequelizeModel(model) {
    // the sequelize models always have static findAll methods
    // (from v3 to v5)
    return !!model && !!model.findAll;
  }

  /**
   * determine if the sequelize model initialized
   * @param {Object} model sequelize model class
   */
  function isInitializedModel(model) {
    // the sequelize-typescript models maybe not initialized
    // and they have to be initialized by `sequelize.addModels([models,...])`
    return !!model && !!model.sequelize;
  }
  ...
  function loadDatabase(config = {}) {
    ...
    const models = [];
    const sequelizeTypescriptModels = [];
    ...
    app.loader.loadToApp(modelDir, target, {
      ...
      filter(model) {
        if (isSequelizeModel(model)) {
          if (isInitializedModel(model)) {
            models.push(model);
            return true;
          }
          sequelizeTypescriptModels.push(model);
        }
        return false;
      },
      initializer(factory) {
        if (typeof factory === 'function') {
          // check if the exports an actual function
          // in this case, you may export a class directly
          if (is.class(factory)) {
            return factory;
          }
    ...
    // for sequelize-typescript, use addModels to initialized them
    if (sequelize.addModels) {
      sequelize.addModels(sequelizeTypescriptModels);
    }
